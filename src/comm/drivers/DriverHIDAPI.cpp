...
DriverHIDAPI::DriverHIDAPI()
  : m_inputBuffer( new uint8_t[ kHIDAPIInputBufferSize ] )
...
bool DriverHIDAPI::read( Transfer& transfer_, uint8_t endpoint_ ) const
{
  int nBytesRead = hid_read( m_pCurrentDevice, m_inputBuffer.get(), kHIDAPIInputBufferSize );
  if( nBytesRead > 0 )
  {
...
bool DriverHIDAPI::write( const Transfer& transfer_, uint8_t endpoint_ ) const
{
  if( transfer_ )
  {
    int nBytesWritten = hid_write( m_pCurrentDevice, transfer_.getDataPtr(), transfer_.getSize() );
    return( nBytesWritten == transfer_.getSize());
  }
