...
DriverMOCK::DriverMOCK()
  : m_inputBuffer( new uint8_t[ kTESTInputBufferSize ] )
{
}
...
bool DriverMOCK::read( Transfer& transfer_, uint8_t endpoint_ ) const
{
...
{
  std::cout << "Packet #" << s_numPacketW << " (" << transfer_.getSize() << " bytes) -> endpoint "
            << static_cast<uint32_t>(endpoint_) << ":" << std::endl;
  
  std::cout << std::setfill('0') << std::internal;
  
  for( int i = 0; i<transfer_.getSize(); i++ )
  {
